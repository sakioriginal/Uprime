...
function init(){
  ...
  activeUnit = player;
  updateLists();
  showDialog("PSE v0.1.4 起動。狩人・衛兵・野生クリーチャー・攻撃指示を追加。", 3200);
}
...
function createPlayer(){
  ...
  const startDir = new THREE.Vector3(0.35, 1.0, 0.25).normalize();
  player.position.copy(startDir.multiplyScalar(surfaceRadius(startDir)+3));
  player.userData = {type:"player", name:"主人公", mind:new Mind("主人公","導き手"), command:null, target:null,
...
function createNPCs(n){
  ...
  for(let i=0;i<n;i++){
    ...
    npc.userData = {type:"npc", name:names[i%names.length], mind, command:null, commandKind:null, target:null, wanderTimer:Math.random()*4,
...
function createCreatures(n){
  const kinds = [
    {name:"野兎", color:0xc9b28a, hp:24, atk:0, hostile:false, speed:8, naturalArmor:.02},
    {name:"鹿", color:0xb47a42, hp:46, atk:0, hostile:false, speed:10, naturalArmor:.04},
    {name:"犬", color:0xd9c29c, hp:52, atk:5, hostile:false, speed:12, naturalArmor:.05},
    {name:"馬", color:0x8b5a2b, hp:90, atk:2, hostile:false, speed:15, naturalArmor:.08},
    {name:"狼", color:0x6f7780, hp:58, atk:9, hostile:true, speed:11, naturalArmor:.1},
    {name:"猪", color:0x5b3d2e, hp:72, atk:11, hostile:true, speed:8, naturalArmor:.16}
  ];
  for(let i=0;i<n;i++){
    const kind = kinds[i % kinds.length];
    const d = new THREE.Vector3().randomDirection(); if(d.y<-.18)d.y=Math.abs(d.y); d.normalize();
    const body = new THREE.Mesh(
      new THREE.CapsuleGeometry(.7, 1.5, 6, 10),
      new THREE.MeshStandardMaterial({color:kind.color})
    );
    body.position.copy(d.multiplyScalar(surfaceRadius(d)+2.1));
    body.userData = {type:"creature", name:kind.name, kind, command:null, target:null, wanderTimer:Math.random()*5,
...
function updatePlayer(dt){
  ...
  if(mv.length()>.01){
    ...
    player.position.add(dir.multiplyScalar(getMoveSpeed(player, 23)*dt));
  ...
  if(commandTarget){
    ...
    if(dir.length()<.85){ commandTarget=null; }
      ...
      player.position.add(moveDir.clone().multiplyScalar(getMoveSpeed(player, 18)*dt));
  ...
  for(const npc of npcs){
    ...
    if(npc.userData.command){
      ...
      if(dir.length()<.7) npc.userData.command=null;
        ...
        npc.position.add(dir.normalize().multiplyScalar(getMoveSpeed(npc, role === "狩人" ? 8.8 : role === "衛兵" ? 8.2 : 7.5)*dt));
...
function updateCreatures(dt){
  for(const c of [...creatures]){
    ...
    orientEntity(c, up, getLocalBasis(up, yaw).forward);
    ...
    if(c.userData.target){
      ...
      if(!t.parent || (t.userData.stats?.hp ?? 100)<=0){ c.userData.target=null; }
        ...
        if(dir.length()>c.userData.combat.range*.55){ c.position.add(dir.normalize().multiplyScalar(getMoveSpeed(c, c.userData.combat.speed)*dt)); }
      }
      ...
      c.userData.command.multiplyScalar(surfaceRadius(c.userData.command)+2.1);
    }
    if(c.userData.command && !c.userData.target){
      ...
      else c.position.add(dir.normalize().multiplyScalar(getMoveSpeed(c, c.userData.kind.speed)*dt));
    }
    const nd=c.position.clone().normalize(); c.position.copy(nd.multiplyScalar(surfaceRadius(nd)+2.1));
  }
}
...
function updateProjectiles(dt){
  for(const arrow of [...projectiles]){
    ...
    if(target && target.parent && (target.userData.stats?.hp ?? 100)>0 && arrow.position.distanceTo(target.position)<1.45){
...
function updateCamera(){
  ...
  if(zoom<2){
    // FPV: 頭の位置。視線は接平面の前方向＋ピッチ。
    const desired = player.position.clone().add(up.clone().multiplyScalar(2.15));
    camera.position.lerp(desired,.55);
    const look = forward.clone().multiplyScalar(Math.cos(pitch)).add(up.clone().multiplyScalar(Math.sin(pitch))).normalize();
    camera.lookAt(camera.position.clone().add(look));
  }else if(zoom<35){
    // TPV: 以前はカメラが高すぎて地表を外していた。
    // 背面・低め・少し下向きにして、必ず地形が画面に入るようにする。
    const camHeight = Math.min(7.5, zoom*.22 + 2.2);
    const desired = player.position.clone()
      .add(up.clone().multiplyScalar(camHeight))
      .add(forward.clone().multiplyScalar(-zoom));
    ...
    // RTS/Planet: デッドゾーン追従。ただし初回は必ずカメラを動かす。
    const desired = player.position.clone()
      .add(up.clone().multiplyScalar(zoom*.55+10))
      .add(forward.clone().multiplyScalar(-zoom*.72));
    const dead = zoom < 125 ? 10 : 25;
    if(camera.position.length() < 1 || camera.position.distanceTo(desired) > dead) camera.position.lerp(desired,.075);
    camera.lookAt(player.position.clone().add(up.clone().multiplyScalar(1.0)));
  }
...
function addPlace(id, name, icon, dir){
  const pos = dir.clone().multiplyScalar(surfaceRadius(dir)+3);
