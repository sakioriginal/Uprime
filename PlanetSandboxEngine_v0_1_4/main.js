...
function makeCreatureModel(kind){
  ...
  for(const x of [-.28,.28]) for(const z of [-.35,.45]){ const leg=new THREE.Mesh(new THREE.CapsuleGeometry(.07,.5,4,6), mat); leg.position.set(x,.18,z); g.add(leg); }
...
function updatePlayer(dt){
  ...
  if(mv.length()>.01){
    ...
    player.position.add(dir.clone().multiplyScalar(getMoveSpeed(player, 9.0)*dt));
  ...
  if(commandTarget){
    ...
    if(dir.length()<.85){ commandTarget=null; }
      ...
      player.position.add(moveDir.clone().multiplyScalar(getMoveSpeed(player, 7.2)*dt));
  ...
  for(const npc of npcs){
    ...
    if(npc.userData.command){
      ...
      if(dir.length()<.7) npc.userData.command=null;
        ...
        npc.position.add(moveDir.clone().multiplyScalar(getMoveSpeed(npc, role === "狩人" ? 3.6 : role === "衛兵" ? 3.2 : 2.8)*dt));
  ...
  for(const c of [...creatures]){
    ...
    if(c.userData.target){
      ...
      if(!t.parent || (t.userData.stats?.hp ?? 100)<=0){ c.userData.target=null; }
        ...
        if(dir.length()>c.userData.combat.range*.55){ const moveDir=dir.normalize(); c.userData.forward=moveDir.clone(); c.position.add(moveDir.clone().multiplyScalar(getMoveSpeed(c, c.userData.combat.speed)*dt)); }
      }
    ...
    if(c.userData.command && !c.userData.target){
      ...
      else { const moveDir=dir.normalize(); c.userData.forward=moveDir.clone(); c.position.add(moveDir.clone().multiplyScalar(getMoveSpeed(c, c.userData.kind.speed)*dt)); }
    }
