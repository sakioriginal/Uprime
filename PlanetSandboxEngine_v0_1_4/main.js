    ...
    if(arrow.userData.free){
      for(const c of creatures){
        ...
        if(arrow.position.distanceTo(c.position)<1.35){
          ...
          arrow.removeFromParent(); projectiles = projectiles.filter(a=>a!==arrow);
      ...
      arrow.removeFromParent(); projectiles = projectiles.filter(a=>a!==arrow); continue;
    }
...
function makeBowModel(scale=1){
  ...
  for(let i=0;i<=28;i++){
    ...
    // Ω字に近い弓形状：中央が手元側、両端が前方へ出る対称カーブ。S字にならない。
    const curve = (-Math.cos((t-.5)*Math.PI*2) * .20) * scale;
  ...
  tipTop.position.set(.20*scale,.625*scale,0);
  const tipBot = tipTop.clone(); tipBot.position.y = -.625*scale;
...
function setupXRHands(){
  ...
  xr.bowArrow.rotation.set(Math.PI/2,0,0);

  xr.sword = makeSwordModel();
  xr.sword.position.set(.02,-.04,-.18);
  xr.sword.rotation.set(0,0,0);
...
function updateXRWorldAnchor(){
  ...
  // プレイヤーの地表法線をVRのY上方向へ合わせ、惑星中心スポーンを防ぐ。
  ...
  const desiredPlayerWorld = new THREE.Vector3(0,0,-0.35); // 目の前に足元が来る程度
