...
let xr = { left:null, right:null, leftGrip:null, rightGrip:null, bow:null, sword:null, bowString:null, drawing:false, drawStart:0, lastRightPos:new THREE.Vector3(), rightVelocity:new THREE.Vector3(), lastSwordHitAt:0, lastTrigger:false, lastGrip:false };
...
function init(){
  ...
  activeUnit = player;
  updateLists();
  showDialog("PSE v0.1.7 起動。VR両手弓・剣スイング判定・VR体験強化を追加。", 3200);
}
...
function setupXRHands(){
  ...
  xr.bow = new THREE.Group();
  const bowMat = new THREE.MeshBasicMaterial({color:0x8b5a2b});
  const stringMat = new THREE.MeshBasicMaterial({color:0xffffff});
  const bowBody = new THREE.Mesh(new THREE.TorusGeometry(.28,.018,8,24,Math.PI*1.35), bowMat);
  bowBody.rotation.z = Math.PI/2;
  const grip = new THREE.Mesh(new THREE.CylinderGeometry(.035,.035,.22,8), bowMat);
  grip.rotation.x = Math.PI/2;
  xr.bowString = new THREE.Mesh(new THREE.CylinderGeometry(.008,.008,.55,6), stringMat);
  xr.bowString.rotation.x = Math.PI/2;
  xr.bowString.position.z = -.08;
  xr.bow.add(bowBody, grip, xr.bowString);
  xr.bow.position.set(0,0,-.08);
  xr.left.add(xr.bow);

  xr.sword = new THREE.Group();
  const blade = new THREE.Mesh(new THREE.BoxGeometry(.045,.045,.9), new THREE.MeshBasicMaterial({color:0xdfe8ff}));
  blade.position.z = -.48;
  const hilt = new THREE.Mesh(new THREE.BoxGeometry(.22,.035,.035), new THREE.MeshBasicMaterial({color:0x553311}));
  xr.sword.add(blade,hilt);
  xr.sword.position.set(0,0,-.12);
  xr.right.add(xr.sword);

  xr.right.addEventListener('selectstart',()=>{ xr.drawing = true; xr.drawStart = performance.now(); });
...
function updateXRHands(dt){
  ...
  if(trigger && !xr.lastTrigger){ xr.drawing = true; xr.drawStart = performance.now(); }
  ...
  if(xr.drawing){
    ...
    const draw = clamp(bowPos.distanceTo(handPos), .08, .85);
    if(xr.bowString) xr.bowString.scale.y = 1 + draw*1.7;
    showVRHint(`🏹 引き ${Math.round(draw*100)} / 離して発射`);
  }else if(xr.bowString){
    xr.bowString.scale.y = 1;
  }

  if(grip && xr.rightVelocity.length() > 2.4 && performance.now()-xr.lastSwordHitAt > 430){
    const hit = findSwordHitTarget();
    if(hit){
      xr.lastSwordHitAt = performance.now();
      const power = clamp(xr.rightVelocity.length()*5.0, 12, 42);
...
function releaseVRBow(){
  ...
  const handPos = getWorldPos(xr.right);
  const draw = clamp(bowPos.distanceTo(handPos), .08, .85);
  // 弓は左手、弦は右手。右手→左手方向に矢を飛ばす。
  let dir = bowPos.clone().sub(handPos).normalize();
  if(dir.lengthSq()<.001) dir = getWorldDir(xr.left);
  const speed = 18 + draw * 62;
  const dmg = 8 + draw * 22 + (player.userData.stats.level-1)*2;
  spawnArrowFree(player, bowPos, dir, speed, dmg);
  awardSkill(player, "弓術", 3);
  showVRHint(`🏹 発射：速度${Math.round(speed)}`);
}
...
function setInputProfile(profile){
  ...
  showDialog(`${profile}操作仕様に切替<br>PC: WASD/マウス/ホイール/QE<br>VR: 左手に弓、右手トリガーで弦を引いて離す。右グリップで剣を握り、振って命中<br>MR: 卓上RTS/手元UI/空間配置`, 2600);
}
