...
# v5.49.0 — UI Customizer / RTS Bird's-eye / Layout Share

- Added top-menu `☰ UI` settings entry.
- Per-widget show/hide controls for sticks, XYAB, hotbar, dial, console, coordinate HUD, combat HUD, SHOP, RTS command bar, top ribbon and side panels.
- UI edit mode: drag movable widgets and resize supported widgets using the lower-right handle.
- Layout positions and sizes are stored as viewport-normalized values so presets remain usable across phone orientations/resolutions.
- Automatic local save of current UI layout.
- Named UI presets stored in browser localStorage.
- JSON export/import for UI presets.
- Shareable UI-layout URL and Web Share support.
- Local QR generation for UI-layout sharing; no external QR service required.
- Shared layout URL auto-imports the layout when opened.
- Added RTS mode and bird's-eye view.
- RTS command bar buttons are configurable. Initial commands: Move / Harvest / Attack / Build / Stop.
- RTS ground tap sets a movement target. Harvest and Attack execute after arrival. Build sets the tapped point as Creator custom origin and opens Building Assist.
