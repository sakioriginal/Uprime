# Universe Engine v5.49.0 — UI Customizer / RTS / Share


## v5.49.0 highlights

- UI widgets can be shown/hidden from the top `☰ UI` menu.
- Movable UI elements can be dragged and resized in UI Edit mode.
- UI layouts support local autosave, named presets, JSON import/export, share links, Web Share, and QR sharing.
- RTS mode now includes a bird's-eye camera and tap-to-command flow for Move, Harvest, Attack, Build, and Stop.
- RTS command buttons can be reassigned from the UI settings panel.
- Existing mobile XYAB action assignment remains available from the mobile action settings button.
