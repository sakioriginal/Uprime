(() => {
  ...
  App.fetchRssFeed = async function fetchRssFeed(feedUrl) {
    ...
    try {
      return await App.fetchRssDirect(feedUrl);
    } catch (e) {
      errors.push(`direct:${e.message}`);
    }

    try {
      return await App.fetchRssViaAllOrigins(feedUrl);
    } catch (e) {
      errors.push(`allorigins:${e.message}`);
    }

    try {
      return await App.fetchRssViaRss2Json(feedUrl);
    } catch (e) {
  ...
  App.wikiOpenSearch = async function wikiOpenSearch(query, limit = 5) {
    try {
      const url =
        `https://ja.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(query)}&limit=${limit}&namespace=0&format=json&origin=*`;
  ...
  App.syncAgentsFromRoleCounts = function syncAgentsFromRoleCounts() {
    ...
    const existing = App.nodes.filter(n => n.isAgentNode);

    if (existing.length < App.agentCount) {
      for (let i = existing.length; i < App.agentCount; i++) {
    ...
    } else if (existing.length > App.agentCount) {
      const remove = existing.slice(App.agentCount);
    ...
    if (!App.followAgentId && App.agents[0]) App.followAgentId = App.agents[0].id;
  };
  ...
  App.roleAction = async function roleAction(agent, targetNode) {
    ...
    if (agent.role === "editor") {
      const result = await App.editorReviewNode(targetNode);
      agent.taskText = `${base} を編集検証`;
      App.addLog(`編集者処理: ${base} (${result})`);
      return;
    }
    ...
    if (agent.role === "miner") {
      ...
      if (result === "exhausted") {
        targetNode.tags = App.uniqueStrings([...(targetNode.tags || []), "mined-out"]);
    ...
    if (agent.role === "interdisciplinary") {
      const wNodes = App.nodes.filter(n => n.is5w2h);
...
/* ===== integrated agent overrides ===== */
App.ensureAgentRuntimeFields = function ensureAgentRuntimeFields(agent) {
  if (!agent) return;
  if (typeof agent.hp !== "number") agent.hp = 100;
  if (typeof agent.hunger !== "number") agent.hunger = 0;
  if (!agent.displayName) {
    const node = App.getNode(agent.id);
    agent.displayName = node ? (node.label || "").replace("\n", " ") : agent.id;
  }
};

const _origSyncAgentsFromRoleCounts = App.syncAgentsFromRoleCounts;
App.syncAgentsFromRoleCounts = function syncAgentsFromRoleCountsIntegrated() {
  _origSyncAgentsFromRoleCounts();
  App.agents.forEach((agent, i) => {
    App.ensureAgentRuntimeFields(agent);
    const node = App.getNode(agent.id);
    if (node) {
      node.roleKey = agent.role;
      node.label = `${agent.displayName || ("Agent" + (i+1))}\n${App.roleLabelJa(agent.role)}`;
    }
  });
};

const _origPickTargetForAgent = App.pickTargetForAgent;
App.pickTargetForAgent = function(agent){
  const picked = _origPickTargetForAgent(agent);
  if (picked && picked.hidden) return null;
  return picked;
};

const _origMoveAgents = App.moveAgents;
App.moveAgents = function(){
  App.agents.forEach(a => App.ensureAgentRuntimeFields(a));
  _origMoveAgents();
};

const _origUpdateAgentThinking = App.updateAgentThinking;
App.updateAgentThinking = function(frame){
  App.agents.forEach(a => App.ensureAgentRuntimeFields(a));
  _origUpdateAgentThinking(frame);
};
