...
/* ===== integrated main overrides ===== */
App.setWebSearchType = function setWebSearchType(type) {
  App.webSearchState.type = type; App.webSubMode = type;
  d.webSearchTabBtn?.classList.toggle("active", type === "web");
  d.imageSearchTabBtn?.classList.toggle("active", type === "image");
  d.youtubeSearchTabBtn?.classList.toggle("active", type === "youtube");
  if (App.mode === "web") App.setMode("web");
};

App.openWebSearchPanel = function(sourceNode){
  if (!sourceNode) return;
  App.webSearchState.sourceNodeId = sourceNode.id;
  App.webSearchState.query = sourceNode.title || sourceNode.label || "";
  App.webSearchState.results = [];
  App.webSearchState.selectedResultIds = new Set();
  if (d.webSearchQueryInput) d.webSearchQueryInput.value = App.webSearchState.query;
  if (d.webSearchStatus) d.webSearchStatus.textContent = "未検索";
  if (d.webSearchResults) d.webSearchResults.innerHTML = "";
  d.webSearchPanel?.classList.remove("hidden");
};
App.closeWebSearchPanel = function(){ d.webSearchPanel?.classList.add("hidden"); };

App.youtubeSearchVideos = async function youtubeSearchVideos(query, maxResults = 8) {
  const apiKey = String(App.integrations?.youtubeApiKey || "").trim();
  if (!apiKey) throw new Error("YouTube API Key が未設定です");
  const q = String(query || "").trim(); if (!q) return [];
  const url = new URL("https://www.googleapis.com/youtube/v3/search");
  url.searchParams.set("part", "snippet"); url.searchParams.set("q", q); url.searchParams.set("type", "video");
  url.searchParams.set("maxResults", String(Math.min(10, Math.max(1, maxResults))));
  url.searchParams.set("safeSearch", "moderate"); url.searchParams.set("regionCode", "JP"); url.searchParams.set("relevanceLanguage", "ja"); url.searchParams.set("key", apiKey);
  ...
  return (data.items || []).filter(item => item?.id?.videoId).map((item, i) => ({
    id: `ytapi_${i}_${item.id.videoId}`, type: "youtube", title: item.snippet?.title || "YouTube",
    url: `https://www.youtube.com/watch?v=${item.id.videoId}`, snippet: item.snippet?.description || "",
    thumbUrl: item.snippet?.thumbnails?.medium?.url || item.snippet?.thumbnails?.default?.url || `https://img.youtube.com/vi/${item.id.videoId}/hqdefault.jpg`,
    videoId: item.id.videoId, channelTitle: item.snippet?.channelTitle || "", publishedAt: item.snippet?.publishedAt || "", source: "youtube"
  }));
};

App.renderWebSearchResults = function(){
  if (!d.webSearchResults) return;
  const results = App.webSearchState.results || [];
  if (!results.length) {
    d.webSearchResults.innerHTML = `<div class="searchCard"><div class="searchCardSnippet">候補なし</div></div>`;
    return;
  }
  d.webSearchResults.innerHTML = results.map((r, i) => {
    const id = r.id || `r_${i}`; const checked = App.webSearchState.selectedResultIds.has(id) ? "checked" : "";
    const thumb = r.thumbUrl || r.imageUrl || ""; const source = r.source || r.type || "";
    return `<div class="searchCard"><label class="searchCardLabel">
      <input type="checkbox" data-result-id="${App.escHtml(id)}" ${checked} />
      ${thumb ? `<img class="searchCardThumb" src="${App.escHtml(thumb)}" alt="">` : ""}
      <div class="searchCardBody">
        <div class="searchCardTitle">${App.escHtml(r.title || r.url || "候補")}</div>
        <div class="searchCardMeta">${App.escHtml(r.url || "")}</div>
        <div class="searchCardSnippet">${App.escHtml(r.snippet || "")}</div>
        <span class="searchSourceTag">${App.escHtml(source)}</span>
      </div>
    </label></div>`;
  }).join("");
};

App.runMockWebSearch = async function(){
  const q = String(d.webSearchQueryInput?.value || App.webSearchState.query || "").trim();
  if (!q) { d.webSearchStatus.textContent = "検索語が空です"; return; }
  App.webSearchState.running = true; App.webSearchState.query = q; d.webSearchStatus.textContent = "候補取得中...";
  let results = [];
  try {
    if (App.webSearchState.type === "web") results = await App.searchWebAll(q);
    else if (App.webSearchState.type === "image") results = await App.searchImageAll(q);
    else if (App.webSearchState.type === "youtube") results = await App.youtubeSearchVideos(q, 8);
    App.webSearchState.results = results; App.webSearchState.selectedResultIds = new Set();
    d.webSearchStatus.textContent = `${results.length}件`; App.renderWebSearchResults();
  } catch (err) {
    console.error(err); App.webSearchState.results = []; App.webSearchState.selectedResultIds = new Set();
    d.webSearchStatus.textContent = `失敗: ${err.message}`; App.renderWebSearchResults();
  } finally { App.webSearchState.running = false; }
};

App.applySelectedWebSearchResults = function(){
  const sourceNode = App.getNode(App.webSearchState.sourceNodeId); if (!sourceNode) return;
  const picked = (App.webSearchState.results || []).filter(r => App.webSearchState.selectedResultIds.has(r.id));
  if (!picked.length) { App.addLog("検索候補が未選択です"); return; }
  for (const item of picked) App.importWebSearchResultAsNode(sourceNode, item);
  App.mergeDuplicateNodes(); App.updateInfo(); App.scheduleAutoSave(); App.addLog(`🌐候補追加: ${picked.length}件`);
};

App.updateInfo = function updateInfo() {
  if (!d.nodeCount || !d.linkCount || !d.selectedInfo) return;
  const visibleCount = (App.nodes || []).filter(n => !n.hidden && !n.isAliasNode).length;
  setText(d.nodeCount, visibleCount);
  setText(d.linkCount, Array.isArray(App.links) ? App.links.length : 0);
  const selected = App.getNode(App.selectedNodeId);
  setText(d.selectedInfo, selected ? (selected.title || selected.label || "なし") : "なし");
  setText(d.agentCountMini, App.agentCount ?? App.agents?.length ?? 0);
  setText(d.mapTypeMini, App.currentMapType || "private");
  setText(d.userNameMini, App.userState?.userName || "guest");
  setText(d.userPointMini, App.userState?.points ?? 0);
  App.updateFollowUI(); App.renderNodeDetail();
  if (selected && document.activeElement !== d.quickInput) d.quickInput.value = selected.title || selected.label || "";
};

App.renderNodeDetail = function renderNodeDetail() {
  if (!App.uiState.showNodeDetailPanel) { hide(d.nodeDetailPanel); return; }
  show(d.nodeDetailPanel);
  d.toggleNodeDetailBtn && (d.toggleNodeDetailBtn.textContent = App.uiState.nodeDetailCollapsed ? "▼" : "▲");
  d.nodeDetailPanel && d.nodeDetailPanel.classList.toggle("collapsed", !!App.uiState.nodeDetailCollapsed);
  const node = App.getNode(App.selectedNodeId);
  if (!node) { setText(d.nodeDetailBody, "未選択"); return; }
  const degree = App.getNodeDegree(node.id), tags = (node.tags || []).join(", ") || "なし", keywords = (node.keywords || []).join(", ") || "なし";
  const host = node.url ? App.tryGetHostname(node.url) : "", sourceHost = node.sourceUrl ? App.tryGetHostname(node.sourceUrl) : "";
  const shapeText = node.shape === "rect" ? "矩形" : "円";
  const mainUrl = node.url || node.mediaUrl || "", sourceUrl = node.sourceUrl || "", sourceType = node.sourceType || "", sourceTitle = node.sourceTitle || "";
  const referrerNode = node.referrerNodeId ? App.getNode(node.referrerNodeId) : null;
  const aliases = (node.aliases || []).join(", ") || "なし", aliasCount = (node.aliases || []).length;
  if (App.uiState.nodeDetailCollapsed) { d.nodeDetailBody.innerHTML = `<div style="font-weight:700;">${App.escHtml(node.title || node.label || "")}</div>`; return; }
  d.nodeDetailBody.innerHTML = `
    <div style="font-size:14px;font-weight:700;margin-bottom:8px;">${App.escHtml(node.title || node.label || "")}</div>
    <div><b>種別:</b> ${App.escHtml(node.category || "normal")}</div>
    <div><b>形:</b> ${shapeText}</div>
    <div><b>座標:</b> (${Math.round(node.x)}, ${Math.round(node.y)}, ${Math.round(node.z || 0)})</div>
    <div><b>固定:</b> ${node.fixed ? "ON" : "OFF"}</div>
    <div><b>表示:</b> ${node.hidden ? "非表示" : "表示"}</div>
    <div><b>リンク数:</b> ${degree}</div>
    <div><b>注目度:</b> ${node.attentionScore ?? 0}</div>
    <div><b>偏差値:</b> ${node.hensachi ?? "なし"}</div>
    <div><b>品質:</b> ${node.qualityScore ?? "なし"}</div>
    <div><b>採掘スコア:</b> ${node.mineScore ?? 0}</div>
    <div><b>採掘回数:</b> ${node.mineHits ?? 0}</div>
    <div><b>作者:</b> ${App.escHtml(node.authorName || "guest")}</div>
    <div><b>マップ:</b> ${App.escHtml(node.mapType || App.currentMapType)}</div>
    <div><b>ポイント:</b> ${node.pointScore ?? 0}</div>
    <div><b>代表名:</b> ${App.escHtml(node.canonicalTitle || node.title || node.label || "")}</div>
    <div><b>別名:</b> ${App.escHtml(aliases)}</div>
    <div><b>別名サブノード:</b> ${aliasCount}</div>
    <div class="detailLabel"><b>ノードURL</b></div>
    ${mainUrl ? `<a class="detailLink" href="${App.escHtml(mainUrl)}" target="_blank" rel="noopener noreferrer">${App.escHtml(mainUrl)}</a><div style="font-size:11px;color:var(--muted);">${App.escHtml(host || "")}</div>` : `<div>なし</div>`}
    <div class="detailLabel"><b>参照元</b></div>
    ${sourceUrl ? `<div>${App.escHtml(sourceTitle || "参照元")}</div><a class="detailLink" href="${App.escHtml(sourceUrl)}" target="_blank" rel="noopener noreferrer">${App.escHtml(sourceUrl)}</a><div style="font-size:11px;color:var(--muted);">${App.escHtml(sourceType || "unknown")} / ${App.escHtml(sourceHost || "")}</div>` : `<div>なし</div>`}
    <div class="detailLabel"><b>参照元ノード</b></div>
    ${referrerNode ? `<a class="detailLink" href="#" data-jump-node-id="${App.escHtml(referrerNode.id)}">${App.escHtml(referrerNode.title || referrerNode.label || "")}</a>` : `<div>なし</div>`}
    <div><b>タグ:</b> ${App.escHtml(tags)}</div>
    <div><b>キーワード:</b> ${App.escHtml(keywords)}</div>
    <div style="margin-top:6px;"><b>概要:</b><br>${App.escHtml(node.summary || "なし")}</div>
    <div style="margin-top:6px;"><b>メモ:</b><br>${App.escHtml(node.memo || "なし")}</div>`;
...
const _origDrawNode = App.drawNode;
App.drawNode = function(n){
  _origDrawNode(n);
};

App.openSettingsTab = App.openSettingsTab || function(tabName){};
const _origOpenSettingsTab = App.openSettingsTab;
App.openSettingsTab = function(tabName){
  _origOpenSettingsTab(tabName);
  d.settingsWorldSection && (d.settingsWorldSection.style.display = tabName === "world" ? "block" : "none");
  d.tabWorldBtn?.classList.toggle("active", tabName === "world");
};
...
App.renderDataSettings = function renderDataSettings() {
  d.settingsDataSection.innerHTML = `
    ...
    <div class="providerGroup">
      <div class="providerRow"><span class="providerLabel">Web</span>
        <label class="providerCheck"><input type="checkbox" id="provWebDDG">DuckDuckGo</label>
        <label class="providerCheck"><input type="checkbox" id="provWebWiki">Wikipedia</label>
        <label class="providerCheck"><input type="checkbox" id="provWebGoogle">Google</label>
        <label class="providerCheck"><input type="checkbox" id="provWebYahoo">Yahoo</label>
      </div>
      <div class="providerRow"><span class="providerLabel">画像</span>
        <label class="providerCheck"><input type="checkbox" id="provImgWiki">Wikipedia</label>
        <label class="providerCheck"><input type="checkbox" id="provImgGoogle">Google</label>
        <label class="providerCheck"><input type="checkbox" id="provImgYahoo">Yahoo</label>
      </div>
    </div>
    <hr style="border:none;border-top:1px solid rgba(255,255,255,.08);margin:10px 0;">
    <h4>Google / YouTube 拡張</h4>
    <div class="row"><label>Google API Key</label><input type="text" id="googleApiKeyProxy" placeholder="AIza..." /></div>
    <div class="row"><label>Google CX</label><input type="text" id="googleCxProxy" placeholder="Custom Search Engine ID" /></div>
    <div class="row"><label>YouTube API Key</label><input type="text" id="youtubeApiKeyProxy" placeholder="AIza..." /></div>
    <button id="rssRefreshNowBtn" type="button">RSS今すぐ更新</button>`;
  const a = d.settingsDataSection.querySelector("#interChkProxy"), b = d.settingsDataSection.querySelector("#autoExpandChkProxy"),
        c = d.settingsDataSection.querySelector("#expandCountRangeProxy"), e = d.settingsDataSection.querySelector("#autoSaveChkProxy"),
        f = d.settingsDataSection.querySelector("#autoSaveSecRangeProxy"), g = d.settingsDataSection.querySelector("#rssEnabledProxy"),
        h = d.settingsDataSection.querySelector("#rssRefreshMinProxy"), i = d.settingsDataSection.querySelector("#rssRefreshNowBtn");
  a.checked = App.uiState.inter; b.checked = App.uiState.autoExpand; c.value = App.uiState.expandCount; e.checked = App.uiState.autoSave; f.value = App.uiState.autoSaveSec; g.checked = App.uiState.rssEnabled; h.value = App.uiState.rssRefreshMin;
  a.addEventListener("input",()=>App.uiState.inter=a.checked); b.addEventListener("input",()=>App.uiState.autoExpand=b.checked); c.addEventListener("input",()=>App.uiState.expandCount=Number(c.value)); e.addEventListener("input",()=>App.uiState.autoSave=e.checked); f.addEventListener("input",()=>App.uiState.autoSaveSec=Number(f.value)); g.addEventListener("input",()=>{App.uiState.rssEnabled=g.checked;App.restartRssTimer();}); h.addEventListener("input",()=>{App.uiState.rssRefreshMin=Number(h.value);App.restartRssTimer();}); i.addEventListener("click", async ()=>{await App.refreshRssFeeds(true);App.updateInfo();});
  const provWebDDG = d.settingsDataSection.querySelector("#provWebDDG"), provWebWiki = d.settingsDataSection.querySelector("#provWebWiki"), provWebGoogle = d.settingsDataSection.querySelector("#provWebGoogle"), provWebYahoo = d.settingsDataSection.querySelector("#provWebYahoo");
  const provImgWiki = d.settingsDataSection.querySelector("#provImgWiki"), provImgGoogle = d.settingsDataSection.querySelector("#provImgGoogle"), provImgYahoo = d.settingsDataSection.querySelector("#provImgYahoo");
  provWebDDG.checked = !!App.searchProviders.web.duckduckgo; provWebWiki.checked = !!App.searchProviders.web.wikipedia; provWebGoogle.checked = !!App.searchProviders.web.google; provWebYahoo.checked = !!App.searchProviders.web.yahoo;
  provImgWiki.checked = !!App.searchProviders.image.wikipedia; provImgGoogle.checked = !!App.searchProviders.image.google; provImgYahoo.checked = !!App.searchProviders.image.yahoo;
  provWebDDG.addEventListener("change",()=>App.searchProviders.web.duckduckgo=provWebDDG.checked); provWebWiki.addEventListener("change",()=>App.searchProviders.web.wikipedia=provWebWiki.checked); provWebGoogle.addEventListener("change",()=>App.searchProviders.web.google=provWebGoogle.checked); provWebYahoo.addEventListener("change",()=>App.searchProviders.web.yahoo=provWebYahoo.checked);
  provImgWiki.addEventListener("change",()=>App.searchProviders.image.wikipedia=provImgWiki.checked); provImgGoogle.addEventListener("change",()=>App.searchProviders.image.google=provImgGoogle.checked); provImgYahoo.addEventListener("change",()=>App.searchProviders.image.yahoo=provImgYahoo.checked);
  const googleApiKeyProxy = d.settingsDataSection.querySelector("#googleApiKeyProxy"), googleCxProxy = d.settingsDataSection.querySelector("#googleCxProxy"), youtubeApiKeyProxy = d.settingsDataSection.querySelector("#youtubeApiKeyProxy");
  googleApiKeyProxy.value = App.integrations.googleApiKey || ""; googleCxProxy.value = App.integrations.googleCx || ""; youtubeApiKeyProxy.value = App.integrations.youtubeApiKey || "";
  googleApiKeyProxy.addEventListener("input",()=>App.integrations.googleApiKey=googleApiKeyProxy.value.trim()); googleCxProxy.addEventListener("input",()=>App.integrations.googleCx=googleCxProxy.value.trim()); youtubeApiKeyProxy.addEventListener("input",()=>App.integrations.youtubeApiKey=youtubeApiKeyProxy.value.trim());
};

App.renderWorldSettings = function renderWorldSettings() {
  d.settingsWorldSection.innerHTML = `<h4>ワールド</h4>
    <div class="row"><label>現在のマップ種別</label><select id="worldMapTypeProxy"><option value="private">プライベート</option><option value="group">グループ</option><option value="world">ワールド</option></select></div>
    <div class="row"><label>ユーザー名</label><input id="worldUserNameProxy" type="text" /></div>
    <div class="row"><label>ポイント口座</label><input id="walletOpenedProxy" type="checkbox" /></div>
    <div class="row"><label>現在ポイント</label><span id="worldPointText">${App.userState.points || 0}</span></div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;"><button id="openSaveSlotPanelProxy" type="button">保存スロット</button><button id="newSaveNameProxy" type="button">保存名を再生成</button></div>
    <div class="miniNote" style="margin-top:8px;">ワールドでは新規作成やコピー時のポイント加算の土台だけ入れています。</div>`;
  const mapSel = d.settingsWorldSection.querySelector("#worldMapTypeProxy"), userInput = d.settingsWorldSection.querySelector("#worldUserNameProxy"), walletChk = d.settingsWorldSection.querySelector("#walletOpenedProxy");
  mapSel.value = App.currentMapType || "private"; userInput.value = App.userState.userName || "guest"; walletChk.checked = !!App.userState.walletOpened;
  mapSel.addEventListener("change",()=>{App.currentMapType = mapSel.value; App.updateInfo(); App.scheduleAutoSave();});
  userInput.addEventListener("input",()=>{App.userState.userName = userInput.value.trim() || "guest"; App.updateInfo();});
  walletChk.addEventListener("input",()=>{App.userState.walletOpened = walletChk.checked; App.scheduleAutoSave();});
  d.settingsWorldSection.querySelector("#openSaveSlotPanelProxy").addEventListener("click",()=>App.openSaveSlotPanel("save"));
  d.settingsWorldSection.querySelector("#newSaveNameProxy").addEventListener("click",()=>{App.currentSaveName = App.defaultSaveName(); App.addLog(`保存名候補: ${App.currentSaveName}`);});
};

const _origRenderAllSettings = App.renderAllSettings;
App.renderAllSettings = function(){ _origRenderAllSettings(); App.renderWorldSettings(); };

App.saveState = function saveState() {
  try { localStorage.setItem(App.SAVE_KEY, JSON.stringify(App.makeSavePayload())); App.addLog("保存しました"); return true; }
  catch(err){ console.error(err); App.addLog("保存に失敗しました"); return false; }
};
App.restoreFromPayload = function restoreFromPayload(data, showLog = true) {
  try {
    App.idCounter = data.idCounter || 1; Object.assign(App.view, data.view || { x:0, y:0, scale:1 });
    App.followAgentId = data.followAgentId || null; App.pendingAgentTargetId = data.pendingAgentTargetId || null; App.zoomedImageNodeId = data.zoomedImageNodeId || null;
    App.currentMapType = data.currentMapType || "private"; App.currentMapId = data.currentMapId || "local-primary"; App.currentSaveName = data.currentSaveName || ""; App.userState = { ...App.userState, ...(data.userState || {}) };
    if (data.integrations && typeof data.integrations === "object") App.integrations = { ...App.integrations, ...data.integrations };
    if (data.searchProviders && typeof data.searchProviders === "object") App.searchProviders = { web: { ...App.searchProviders.web, ...(data.searchProviders.web || {}) }, image: { ...App.searchProviders.image, ...(data.searchProviders.image || {}) } };
    if (data.roleCounts && typeof data.roleCounts === "object") App.roleCounts = { ...App.roleCounts, ...data.roleCounts };
    if (typeof data.totalAgentCount === "number") App.totalAgentCount = data.totalAgentCount;
    if (data.uiState && typeof data.uiState === "object") Object.assign(App.uiState, data.uiState);
    App.nodes = Array.isArray(data.nodes) ? data.nodes.map(App.restoreNodeRuntime) : [];
    App.links = Array.isArray(data.links) ? data.links.map(l => ({...l})) : [];
    App.agents = Array.isArray(data.agents) ? data.agents.map(a => ({...a})) : [];
    if (data.panelPins) Object.assign(App.panelPins, data.panelPins);
    if (data.panelPositions) Object.assign(App.panelPositions, data.panelPositions);
    App.rssState.seenHeadlines = new Set(data.rssSeenHeadlines || []);
    App.nodes.forEach(n => { if (n.imageSrc) App.restoreImageNode(n); if (n.youtubeVideoId) n.shape = "rect"; });
    App.renderAllSettings(); App.ensure5W2HNodes(); App.syncAgentsFromRoleCounts(); App.applyPanelLayout(); App.updateZoomHud(); App.updateInfo(); App.syncAllAliasChildNodes?.();
    if (showLog) App.addLog("読込しました");
    return true;
  } catch(err){ console.error(err); App.addLog("読込に失敗しました"); return false; }
};
App.loadState = function loadState(showLog = true) {
  const raw = localStorage.getItem(App.SAVE_KEY); if (!raw) return false;
  try { return App.restoreFromPayload(JSON.parse(raw), showLog); } catch(err){ console.error(err); App.addLog("読込に失敗しました"); return false; }
};

App.openSaveSlotPanel = function(mode = "save") {
  d.saveSlotPanel?.classList.remove("hidden");
  if (d.mapTypeSelect) d.mapTypeSelect.value = App.currentMapType || "private";
  if (d.userNameInput) d.userNameInput.value = App.userState?.userName || "guest";
  if (d.saveNameInput) d.saveNameInput.value = App.currentSaveName || App.defaultSaveName();
  if (d.saveSlotModeText) d.saveSlotModeText.textContent = mode === "load" ? "読込モード" : "保存モード";
  App.renderSaveSlotList(mode);
};
App.closeSaveSlotPanel = function(){ d.saveSlotPanel?.classList.add("hidden"); };
App.renderSaveSlotList = function(mode = "save") {
  if (!d.saveSlotList) return;
  const slots = [...App.getSlotStore()].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
  if (!slots.length) { d.saveSlotList.innerHTML = `<div class="slotItem"><div class="slotMeta">保存データなし</div></div>`; return; }
  d.saveSlotList.innerHTML = slots.map((slot, idx) => `<div class="slotItem"><div class="searchResultTitle">${App.escHtml(slot.saveName || `slot-${idx + 1}`)}</div><div class="slotMeta">${App.escHtml(slot.mapType || "private")} / ${App.escHtml(slot.userName || "guest")} / ${new Date(slot.updatedAt || Date.now()).toLocaleString("ja-JP")}</div><div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap;"><button type="button" data-slot-index="${idx}" data-slot-action="load">読込</button>${mode === "save" ? `<button type="button" data-slot-index="${idx}" data-slot-action="overwrite">上書き</button>` : ""}<button type="button" data-slot-index="${idx}" data-slot-action="delete">削除</button></div></div>`).join("");
};
App.saveToNamedSlot = function() {
  const saveName = String(d.saveNameInput?.value || "").trim() || App.defaultSaveName(), mapType = d.mapTypeSelect?.value || App.currentMapType || "private", userName = String(d.userNameInput?.value || "").trim() || "guest";
  App.currentMapType = mapType; App.userState.userName = userName; App.currentSaveName = saveName;
  const payload = App.makeSavePayload(), slots = App.getSlotStore(), slotData = { saveName, mapType, userName, updatedAt: Date.now(), payload };
  const found = slots.findIndex(s => s.saveName === saveName && s.mapType === mapType); if (found >= 0) slots[found] = slotData; else slots.push(slotData);
  App.setSlotStore(slots); App.saveState(); App.renderSaveSlotList("save"); App.updateInfo(); App.addLog(`保存スロット保存: ${saveName}`);
};
App.loadFromSlotIndex = function(index) {
  const slots = [...App.getSlotStore()].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)); const slot = slots[index]; if (!slot?.payload) return false;
  const ok = App.restoreFromPayload(slot.payload, true);
  if (ok) { App.currentSaveName = slot.saveName || ""; App.currentMapType = slot.mapType || "private"; App.userState.userName = slot.userName || App.userState.userName || "guest"; App.updateInfo(); requestAnimationFrame(()=>App.updateInfo()); }
  return ok;
};
App.deleteSlotIndex = function(index) {
  const sorted = [...App.getSlotStore()].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)); const target = sorted[index]; if (!target) return;
  const remain = App.getSlotStore().filter(s => !(s.saveName === target.saveName && s.mapType === target.mapType));
  App.setSlotStore(remain); App.renderSaveSlotList("save"); App.addLog(`保存スロット削除: ${target.saveName}`);
};

const _origSetMode = App.setMode;
App.setMode = function(mode){
  _origSetMode(mode);
  d.webModeBtn?.classList.toggle("active", mode === "web");
  d.pruneModeBtn?.classList.toggle("active", mode === "prune");
  let label = mode === "view" ? "閲覧" : mode === "edit" ? "編集" : mode === "link" ? "リンク" : mode === "web" ? `🌐 ${App.webSubMode || "web"}` : mode === "prune" ? "✂️ 剪定" : mode;
  setText(d.modeBadge, `モード: ${label}`);
};

const _origBindEvents = App.bindEvents;
App.bindEvents = function(){
  _origBindEvents();
  d.quickInputClearBtn?.addEventListener("click",()=>{ if (d.quickInput) d.quickInput.value = ""; });
  d.webModeBtn?.addEventListener("click",()=>{ App.setWebSearchType(App.webSubMode || "web"); App.setMode("web"); });
  d.pruneModeBtn?.addEventListener("click",()=>App.setMode("prune"));
  d.tabWorldBtn?.addEventListener("click",()=>App.openSettingsTab("world"));
  d.closeWebSearchBtn?.addEventListener("click",()=>App.closeWebSearchPanel());
  d.webSearchTabBtn?.addEventListener("click",()=>App.setWebSearchType("web"));
  d.imageSearchTabBtn?.addEventListener("click",()=>App.setWebSearchType("image"));
  d.youtubeSearchTabBtn?.addEventListener("click",()=>App.setWebSearchType("youtube"));
  d.runWebSearchBtn?.addEventListener("click",()=>App.runMockWebSearch());
  d.applyWebSearchSelectionBtn?.addEventListener("click",()=>App.applySelectedWebSearchResults());
  d.webSearchResults?.addEventListener("change",(e)=>{ const t = e.target; if (!(t instanceof HTMLInputElement)) return; const id = t.getAttribute("data-result-id"); if (!id) return; if (t.checked) App.webSearchState.selectedResultIds.add(id); else App.webSearchState.selectedResultIds.delete(id); });
  d.closeSaveSlotPanelBtn?.addEventListener("click",()=>App.closeSaveSlotPanel());
  d.saveBtn?.addEventListener("click",()=>App.openSaveSlotPanel("save"));
  d.loadBtn?.addEventListener("click",()=>App.openSaveSlotPanel("load"));
  d.saveToSlotBtn?.addEventListener("click",()=>App.saveToNamedSlot());
  d.refreshSlotListBtn?.addEventListener("click",()=>App.renderSaveSlotList("save"));
  d.saveSlotList?.addEventListener("click",(e)=>{ const t = e.target; if (!(t instanceof HTMLButtonElement)) return; const idx = Number(t.getAttribute("data-slot-index")); const action = t.getAttribute("data-slot-action"); if (!Number.isFinite(idx)) return; if (action === "load") { const ok = App.loadFromSlotIndex(idx); if (ok) { App.closeSaveSlotPanel(); App.updateInfo(); App.scheduleAutoSave(); } } else if (action === "overwrite") { const slots = [...App.getSlotStore()].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)); const slot = slots[idx]; if (!slot) return; d.saveNameInput && (d.saveNameInput.value = slot.saveName || App.defaultSaveName()); d.mapTypeSelect && (d.mapTypeSelect.value = slot.mapType || "private"); App.saveToNamedSlot(); } else if (action === "delete") App.deleteSlotIndex(idx); });
  d.nodeDetailBody?.addEventListener("click",(e)=>{ const link = e.target.closest("[data-jump-node-id]"); if (!link) return; e.preventDefault(); const nodeId = link.getAttribute("data-jump-node-id"); const node = App.getNode(nodeId); if (!node) return; App.focusNode(node); App.updateInfo(); });
};

const _origOnCanvasPointerDown = App.onCanvasPointerDown;
App.onCanvasPointerDown = function(e){
  const p = App.pointerPosFromEvent(e); const w = App.screenToWorld(p.x,p.y); const node = App.hitTestNode(w.x,w.y);
  if (node) App.bringNodeToFront(node);
  if (App.mode === "prune") { if (node && !node.is5w2h && !node.isAliasNode) { App.deleteNode(node.id); App.updateInfo(); App.scheduleAutoSave(); } return; }
  if (App.mode === "web") { if (node && !node.isAgentNode) { App.selectedNodeId = node.id; App.openWebSearchPanel(node); App.updateInfo(); } else if (!node) { App.selectedNodeId = null; App.updateInfo(); } return; }
  _origOnCanvasPointerDown(e);
};

const _origQuickAddClick = d.quickAddBtn?.onclick;
if (d.quickAddBtn) {
  d.quickAddBtn.replaceWith(d.quickAddBtn.cloneNode(true));
}


const __origDraw = App.draw;
App.draw = function(){
  App.visibleNodeSet = App.getVisibleNodeSet();
  ctx.clearRect(0,0,App.W,App.H);
  App.drawGrid(); App.drawLinks();
  const sorted = [...(App.nodes || [])].sort((a,b)=> (a.zIndex||0) - (b.zIndex||0));
  for (const n of sorted) {
    if (n.hidden) continue;
    if (n.isAliasNode) {
      const p = App.worldToScreen(n.x,n.y), rr = (n.r || 14) * App.view.scale;
      ctx.save(); ctx.beginPath(); ctx.arc(p.x,p.y,rr,0,Math.PI*2); ctx.fillStyle="#3d4b63"; ctx.fill(); ctx.strokeStyle="#b9d7ff"; ctx.lineWidth=1.4; ctx.stroke();
      const title = String(n.title || n.label || "").slice(0, 12);
      ctx.font = `${Math.max(9,10*App.view.scale)}px sans-serif`; ctx.fillStyle="#dbeafe"; ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.fillText(title,p.x,p.y);
      ctx.restore();
    } else {
      App.drawNode(n);
    }
...
const __origPhysicsStep = App.physicsStep;
App.physicsStep = function(){
  __origPhysicsStep();
  App.nodes.forEach(n => {
    if (!n.isAliasNode) return;
    const parent = App.getNode(n.parentNodeId);
    if (!parent) return;
    const aliases = (parent.aliases || []).filter(Boolean);
    const i = Math.max(0, n.aliasIndex || 0);
    const angle = (Math.PI * 2 / Math.max(1, aliases.length)) * i;
    const radius = Math.max(70, (parent.shape === "rect" ? Math.max(parent.width || 120, parent.height || 80) * 0.5 : (parent.r || 24)) + 52);
...
const __origBoot = App.boot;
App.boot = async function(){
  await __origBoot();
  requestAnimationFrame(()=>{ App.updateInfo(); App.draw(); });
};
