(() => {
  ...
  App.applyPanelLayout = function applyPanelLayout() {
    if (!d.topBarPanel || !d.settingsPanel || !d.nodeDetailPanel || !d.nodePropPanel) return;
    ...
    if (App.panelPins?.nodePropPanel) {
      d.nodePropPanel.style.right = "8px";
      d.nodePropPanel.style.left = "auto";
      d.nodePropPanel.style.top = "380px";
      d.nodePropPanel.style.bottom = "auto";
    } else {
      d.nodePropPanel.style.right = "auto";
      d.nodePropPanel.style.left = `${App.panelPositions.nodePropPanel.left}px`;
      d.nodePropPanel.style.top = `${App.panelPositions.nodePropPanel.top}px`;
      d.nodePropPanel.style.bottom = "auto";
    }

    if (d.pinTopBarBtn) d.pinTopBarBtn.classList.toggle("pinned", !!App.panelPins.topBarPanel);
    if (d.pinSettingsBtn) d.pinSettingsBtn.classList.toggle("pinned", !!App.panelPins.settingsPanel);
    if (d.pinDetailBtn) d.pinDetailBtn.classList.toggle("pinned", !!App.panelPins.nodeDetailPanel);
    if (d.pinPropBtn) d.pinPropBtn.classList.toggle("pinned", !!App.panelPins.nodePropPanel);
  };
  ...
  App.resize = function resize() {
    ...
    if (ctx) ctx.setTransform(App.DPR, 0, 0, App.DPR, 0, 0);
  ...
  App.renderNodeDetail = function renderNodeDetail() {
    ...
    const scaleFactor = Math.round(App.getNodeScaleFactor(node) * 100) / 100;
    ...
    d.nodeDetailBody.innerHTML = `
      ...
      <div><b>表示:</b> ${node.visible === false ? "OFF" : "ON"}</div>
      ...
      <div><b>ユーザー重み:</b> ${node.userFocusScore ?? 0}</div>
      <div><b>表示重み:</b> ${displayWeight}</div>
      <div><b>サイズ倍率:</b> ${scaleFactor}</div>
  ...
  App.renderNodePropPanel = function renderNodePropPanel() {
    if (!App.uiState.showNodePropPanel) {
      hide(d.nodePropPanel);
      return;
    }
    show(d.nodePropPanel);
    App.loadNodePropsToPanel(App.getNode(App.selectedNodeId));
  };

  App.updateInfo = function updateInfo() {
    setText(d.nodeCount, App.nodes?.filter(n => !n.isAgentNode).length ?? 0);
  ...
  App.closeEditor = function closeEditor() {
    if (d.nodeEditor) {
      d.nodeEditor.style.display = "none";
  ...
  App.drawLinks = function drawLinks() {
    for (const l of App.links || []) {
      ...
      const aVisible = a.isAgentNode || (a.visible !== false && App.visibleNodeSet.has(a.id));
      const bVisible = b.isAgentNode || (b.visible !== false && App.visibleNodeSet.has(b.id));
      ...
      if (l.id === App.selectedLinkId) {
        ctx.strokeStyle = "#ff8f8f";
        ctx.lineWidth = 3;
      } else if (l.id === App.hoveredLinkId && App.mode === "prune") {
        ctx.strokeStyle = "#ffd166";
        ctx.lineWidth = 2.5;
      } else if (l.type === "5w2h-bridge") {
  ...
  App.drawNode = function drawNode(n) {
    if (!n.isAgentNode && n.visible === false) return;
    if (!n.isAgentNode && !App.visibleNodeSet.has(n.id)) return;

    const p = App.worldToScreen(n.x, n.y);
    const scaleFactor = App.getNodeScaleFactor(n);

    if (n.shape === "rect") {
      const isZoomed = App.zoomedImageNodeId === n.id;
      let rw = (n.width || 120) * scaleFactor * App.view.scale;
      let rh = (n.height || 80) * scaleFactor * App.view.scale;
      if (isZoomed) {
        rw *= 2;
        rh *= 2;
      }
      ...
      if (App.mode === "prune" && App.hoveredNodeId === n.id) {
        ctx.strokeStyle = "#ff8f8f";
        ctx.lineWidth = 3;
      }
      App.roundRect(p.x - rw / 2, p.y - rh / 2, rw, rh, 12);
    ...
    const rr = (n.r || 24) * scaleFactor * App.view.scale;
    ...
    if (n.isAgentNode && zoom > 0.75) {
      ...
      if (agent) {
        const status = `${App.roleLabelJa(agent.role)} | ${agent.mode === "move" ? "移動" : agent.mode === "work" ? "作業" : "待機"}`;
  ...
  App.physicsStep = function physicsStep() {
    ...
    App.nodes.forEach(n => {
      ...
      if (!n.isAgentNode) App.decayUserFocusScore(n);
    });
    ...
    App.nodes.forEach(n => {
      ...
      if (!n.isAgentNode && n.visible !== false) {
        n.x += n.vx;
        n.y += n.vy;
        n.vx *= damping;
        n.vy *= damping;
  ...
  App.fitToAll = function fitToAll() {
    const visibleNodes = (App.nodes || []).filter(n => n.visible !== false);
    if (!visibleNodes.length) return;

    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    visibleNodes.forEach(n => {
      const scaleFactor = App.getNodeScaleFactor(n);
      const hw = n.shape === "rect" ? ((n.width || 120) * scaleFactor) / 2 : (n.r || 20) * scaleFactor;
      const hh = n.shape === "rect" ? ((n.height || 80) * scaleFactor) / 2 : (n.r || 20) * scaleFactor;
  ...
  App.alignNodes = function alignNodes() {
    const arr = App.nodes.filter(n => !n.isAgentNode && !n.is5w2h && n.visible !== false);
    ...
    const startY = -Math.ceil(arr.length / cols) * gapY / 2;
  ...
  App.resetMap = function resetMap() {
    ...
    seeds.forEach(s => {
      const n = App.makeNode({
        ...
        r: 24, color: "#2b3344", textColor: "#eaf1ff", category: "normal", visible: true
      });
      n.title = s.label;
      n.summary = s.memo;
      n.rootNodeId = n.id;
  ...
  App.renderViewSettings = function renderViewSettings() {
    d.settingsViewSection.innerHTML = `
      ...
      <div class="row"><label>ノードプロパティ表示</label><input type="checkbox" id="showNodePropPanelProxy"></div>
      <div class="row"><label>選択時に重みアップ</label><input type="checkbox" id="boostOnSelectProxy"></div>
    `;
    ...
    const showProp = d.settingsViewSection.querySelector("#showNodePropPanelProxy");
    const boostOnSelect = d.settingsViewSection.querySelector("#boostOnSelectProxy");
    ...
    showProp.checked = !!App.uiState.showNodePropPanel;
    boostOnSelect.checked = !!App.uiState.boostOnSelect;
    ...
    showProp.addEventListener("input", () => { App.uiState.showNodePropPanel = showProp.checked; App.renderNodePropPanel(); });
    boostOnSelect.addEventListener("input", () => { App.uiState.boostOnSelect = boostOnSelect.checked; });
  };
  ...
  App.renderPhysicsSettings = function renderPhysicsSettings() {
    d.settingsPhysicsSection.innerHTML = `
      ...
      <div class="row"><label>選択ブースト量</label><input type="range" id="selectBoostAmountProxy" min="1" max="40"></div>
      <div class="row"><label>タップブースト量</label><input type="range" id="tapBoostAmountProxy" min="1" max="30"></div>
    `;
    ...
    const e = d.settingsPhysicsSection.querySelector("#selectBoostAmountProxy");
    const f = d.settingsPhysicsSection.querySelector("#tapBoostAmountProxy");

    a.checked = App.uiState.physics;
    b.value = App.uiState.repel;
    c.value = App.uiState.spring;
    dd.value = App.uiState.damping;
    e.value = App.uiState.selectBoostAmount;
    f.value = App.uiState.tapBoostAmount;
    ...
    e.addEventListener("input", () => { App.uiState.selectBoostAmount = Number(e.value); });
    f.addEventListener("input", () => { App.uiState.tapBoostAmount = Number(f.value); });
  };
  ...
  App.renderDataSettings = function renderDataSettings() {
    ...
    i.addEventListener("click", () => { App.refreshRssFeeds(true); });
  };
  ...
  /* =========================
     Input / pointer
  ========================= */
  function zoomAtScreen(screenX, screenY, scaleMul) {
    const before = App.screenToWorld(screenX, screenY);
    App.view.scale = Math.max(0.1, Math.min(3, App.view.scale * scaleMul));
    const after = App.screenToWorld(screenX, screenY);
    App.view.x += before.x - after.x;
    App.view.y += before.y - after.y;
    App.updateZoomHud();
  }

  function handleCanvasTap(world, screen) {
    const hitNode = App.hitTestNode(world.x, world.y);
    const hitLink = !hitNode ? App.hitTestLink(world.x, world.y) : null;

    if (App.mode === "prune") {
      if (hitNode && !hitNode.is5w2h) {
        App.deleteNode(hitNode.id);
        App.updateInfo();
        App.scheduleAutoSave();
        return;
      }
      if (hitLink) {
        App.deleteLink(hitLink.id);
        App.updateInfo();
        App.scheduleAutoSave();
        return;
      }
    ...
    if (App.mode === "link") {
      ...
      if (!App.linkingFromId) {
        App.linkingFromId = hitNode.id;
        App.selectNode(hitNode, "tap");
        App.updateInfo();
        App.addLog(`リンク開始: ${hitNode.title || hitNode.label}`);
        return;
      }
      const from = App.getNode(App.linkingFromId);
      if (from && from.id !== hitNode.id) {
        App.addLinkSmart(from, hitNode);
      }
      App.linkingFromId = null;
      App.selectNode(hitNode, "tap");
      App.updateInfo();
    ...
    if (App.mode === "view") {
      if (hitNode?.isAgentNode) {
        App.pendingAgentTargetId = hitNode.id;
        App.selectNode(hitNode, "tap");
        App.followAgentId = hitNode.id;
        App.updateInfo();
        App.addLog(`エージェント選択: ${hitNode.label.replace("\n", " ")}`);
        return;
      }

      if (App.pendingAgentTargetId && hitNode) {
        const agent = App.agents.find(a => a.id === App.pendingAgentTargetId);
        if (agent) {
          agent.targetNodeId = hitNode.id;
          App.addLog(`移動指定: ${App.getNode(agent.id)?.label.replace("\n"," ")} → ${hitNode.title || hitNode.label}`);
        }
        App.pendingAgentTargetId = null;
        App.selectNode(hitNode, "tap");
        App.updateInfo();
      ...
      if (hitNode) {
        if (hitNode.youtubeVideoId) {
          App.openYoutubeOverlay(hitNode);
        } else if (hitNode.shape === "rect" && hitNode.imageSrc) {
          App.zoomedImageNodeId = App.zoomedImageNodeId === hitNode.id ? null : hitNode.id;
        }
        App.selectNode(hitNode, "tap");
        App.updateInfo();
        return;
    ...
    if (App.mode === "edit") {
      if (hitNode) {
        App.selectNode(hitNode, "tap");
        App.updateInfo();
        if (App.dom.quickInput && document.activeElement !== App.dom.quickInput) {
          App.dom.quickInput.value = hitNode.title || hitNode.label || "";
        }
      } else {
        const text = String(d.quickInput?.value || "").trim() || "新規ノード";
        const created = App.createNodeAtWorld(world.x, world.y, text);
        App.selectNode(created, "select");
        App.openEditor(created, screen.x, screen.y);
        App.updateInfo();
        App.scheduleAutoSave();
      }
    }
  ...
  function setupCanvasPointerEvents() {
    ...
    d.canvas.addEventListener("pointerdown", (e) => {
      ...
      const node = App.hitTestNode(world.x, world.y);
      const link = !node ? App.hitTestLink(world.x, world.y) : null;

      App.hoveredNodeId = node?.id || null;
      App.hoveredLinkId = link?.id || null;

      if (App.mode === "edit" && node && App.hitResizeHandle(node, world.x, world.y)) {
        App.resizingNodeId = node.id;
        return;
      }

      if (node && App.mode !== "prune") {
        App.draggingNodeId = node.id;
        App.dragOffsetWorldX = world.x - node.x;
        App.dragOffsetWorldY = world.y - node.y;
        App.selectNode(node, "tap");
        App.updateInfo();
      } else {
        App.panning = true;
      }
      ...
      if (App.mode === "edit" && !node) {
        ...
        App.longPressTimer = setTimeout(() => {
          const created = App.createNodeAtWorld(world.x, world.y, String(d.quickInput?.value || "").trim() || "新規ノード");
          App.selectNode(created, "select");
          App.openEditor(created, sx, sy);
          App.updateInfo();
          App.scheduleAutoSave();
        }, App.LONG_PRESS_MS);
      }
    ...
    d.canvas.addEventListener("pointermove", (e) => {
      ...
      if (!App.pointerDown) {
        const hoverNode = App.hitTestNode(world.x, world.y);
        const hoverLink = !hoverNode ? App.hitTestLink(world.x, world.y) : null;
        App.hoveredNodeId = hoverNode?.id || null;
        App.hoveredLinkId = hoverLink?.id || null;
        return;
      }

      const moved = Math.hypot(pos.x - App.lastPointer.x, pos.y - App.lastPointer.y);
      if (moved > 8) App.clearLongPressTimer();

      if (App.resizingNodeId) {
        const node = App.getNode(App.resizingNodeId);
        if (node) {
          const scale = App.getNodeScaleFactor(node);
          const dx = (world.x - node.x) / Math.max(scale, 0.001);
          const dy = (world.y - node.y) / Math.max(scale, 0.001);
          node.width = Math.max(20, Math.abs(dx) * 2);
          node.height = Math.max(20, Math.abs(dy) * 2);
      ...
      if (App.draggingNodeId) {
        ...
        if (node && !node.fixed) {
          node.x = world.x - App.dragOffsetWorldX;
          node.y = world.y - App.dragOffsetWorldY;
          node.vx = 0;
          node.vy = 0;
          node.updatedAt = Date.now();
        }
      ...
      if (App.panning) {
        const prevWorld = App.screenToWorld(App.lastPointer.x, App.lastPointer.y);
        const currWorld = App.screenToWorld(pos.x, pos.y);
        App.view.x -= (currWorld.x - prevWorld.x);
        App.view.y -= (currWorld.y - prevWorld.y);
      }
    ...
    const endPointer = (e) => {
      ...
      const wasDragging = !!App.draggingNodeId;
      const wasResizing = !!App.resizingNodeId;
      const wasPanning = !!App.panning;
      ...
      const now = Date.now();
      const isDoubleTap = (now - App.lastTapTime) <= App.DOUBLE_TAP_MS &&
        App.isNearPoint(pos.x, pos.y, App.lastTapX, App.lastTapY);

      if (!wasDragging && !wasResizing && !wasPanning) {
        handleCanvasTap(world, pos);
      } else if (wasDragging || wasResizing) {
        App.scheduleAutoSave();
      }

      const node = App.hitTestNode(world.x, world.y);
      if (App.mode === "edit" && node && isDoubleTap && !node.is5w2h) {
        App.openEditor(node, pos.x, pos.y);
      }
    ...
    d.canvas.addEventListener("pointerup", endPointer);
    d.canvas.addEventListener("pointercancel", () => {
      App.clearLongPressTimer();
      App.draggingNodeId = null;
      App.resizingNodeId = null;
      App.panning = false;
    ...
    d.canvas.addEventListener("wheel", (e) => {
      e.preventDefault();
      const mul = e.deltaY < 0 ? 1.12 : 0.89;
      const pos = App.pointerPosFromEvent(e);
      zoomAtScreen(pos.x, pos.y, mul);
    }, { passive: false });
  }
  ...
  ========================= */
  function setupStick() {
    if (!d.stickWrap || !d.stickKnob) return;

    const rectOfWrap = () => d.stickWrap.getBoundingClientRect();

    const resetKnob = () => {
      App.stickState.active = false;
      App.stickState.dx = 0;
      App.stickState.dy = 0;
      d.stickKnob.style.left = `${(d.stickWrap.clientWidth - d.stickKnob.clientWidth) / 2}px`;
      d.stickKnob.style.top = `${(d.stickWrap.clientHeight - d.stickKnob.clientHeight) / 2}px`;
    };

    resetKnob();

    const moveStick = (clientX, clientY) => {
      const rect = rectOfWrap();
      ...
      const knobLeft = rect.width / 2 - d.stickKnob.clientWidth / 2 + dx;
      const knobTop = rect.height / 2 - d.stickKnob.clientHeight / 2 + dy;
      d.stickKnob.style.left = `${knobLeft}px`;
      d.stickKnob.style.top = `${knobTop}px`;
    };
    ...
    d.stickWrap.addEventListener("pointerup", resetKnob);
    d.stickWrap.addEventListener("pointercancel", resetKnob);
  }
  ...
  function setupMainButtons() {
    ...
    d.loadBtn?.addEventListener("click", () => App.loadState(true));
    d.fitBtn?.addEventListener("click", () => App.fitToAll());
    d.alignBtn?.addEventListener("click", () => App.alignNodes());
    ...
    d.followBtn?.addEventListener("click", () => {
      if (App.followAgentId) App.followAgentId = null;
      else if (App.agents[0]) App.followAgentId = App.agents[0].id;
      App.updateInfo();
    });
    ...
    d.quickAddBtn?.addEventListener("click", () => {
      const text = String(d.quickInput?.value || "").trim() || "新規ノード";
      const node = App.createNodeAtWorld(App.view.x, App.view.y, text);
    ...
    d.quickEditBtn?.addEventListener("click", () => {
      const node = App.getNode(App.selectedNodeId);
      if (!node) return;
      if (String(d.quickInput?.value || "").trim()) {
        App.renameNode(node, d.quickInput.value);
        App.loadNodePropsToPanel(node);
        App.updateInfo();
        App.scheduleAutoSave();
      } else {
        App.openEditor(node, App.W / 2, 120);
      }
    });

    d.quickInput?.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        if (App.mode === "edit") {
          const node = App.getNode(App.selectedNodeId);
          if (node) {
            ...
            const created = App.createNodeAtWorld(App.view.x, App.view.y, d.quickInput.value);
            App.selectNode(created, "select");
            App.updateInfo();
            App.scheduleAutoSave();
          }
    ...
    d.zoomInBtn?.addEventListener("click", () => zoomAtScreen(App.W / 2, App.H / 2, 1.15));
    d.zoomOutBtn?.addEventListener("click", () => zoomAtScreen(App.W / 2, App.H / 2, 0.87));
  }
  ...
  function setupEditorButtons() {
    d.nodeSaveBtn?.addEventListener("click", () => {
      ...
      if (!node) return;
      App.renameNode(node, d.nodeLabelInput.value);
      node.memo = d.nodeMemoInput.value || "";
      node.color = d.nodeColorInput.value || node.color;
      node.updatedAt = Date.now();
      App.closeEditor();
      App.loadNodePropsToPanel(node);
      App.updateInfo();
    ...
    d.propApplyBtn?.addEventListener("click", () => {
      const node = App.getNode(App.selectedNodeId);
      if (!node) return;
      if (App.applyNodePropsFromPanel(node)) {
        App.updateInfo();
        App.scheduleAutoSave();
      }
    });

    d.propDeleteNodeBtn?.addEventListener("click", () => {
      const node = App.getNode(App.selectedNodeId);
      if (!node) return;
      App.deleteNode(node.id);
      App.updateInfo();
    ...
    d.propCutLinksBtn?.addEventListener("click", () => {
      const node = App.getNode(App.selectedNodeId);
      if (!node) return;
      App.cutLinksOfNode(node.id);
      App.updateInfo();
      App.scheduleAutoSave();
    });
  }
  ...
  /* =========================
     Tabs / detail / overlay / panel pins
  ========================= */
  function setupOtherUi() {
    ...
    d.pinPropBtn?.addEventListener("click", () => {
      App.panelPins.nodePropPanel = !App.panelPins.nodePropPanel;
      App.applyPanelLayout();
    });
    ...
    App.makePanelDraggable(d.nodePropPanel, "nodePropPanel");
  }
