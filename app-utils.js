...
(() => {
  ...
  App.sanitizeGeneratedLabel = function sanitizeGeneratedLabel(text) {
    ...
    const chunks =
      s.match(/[A-Za-z]+[0-9]*|[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]+/gu) || [];
  ...
  App.createInterdisciplinaryNodeBetween = function createInterdisciplinaryNodeBetween(a, b) {
    ...
    const label = App.sanitizeGeneratedLabel(
      `${normalNode.title || normalNode.label} ${wNode.title || wNode.label}`
    );
  ...
  App.addLinkSmart = function addLinkSmart(a, b) {
    ...
    if (App.uiState.inter && (aIs5 !== bIs5)) {
      ...
      if (inter) {
        if (!App.linkExists(a.id, inter.id)) {
          App.links.push(App.makeLink(a.id, inter.id, "5w2h-bridge", 1, true));
        }
        if (!App.linkExists(inter.id, b.id)) {
          App.links.push(App.makeLink(inter.id, b.id, "5w2h-bridge", 1, true));
        }
        return;
      }
    }
  ...
  App.mergeDuplicateNodes = function mergeDuplicateNodes() {
    ...
    for (const node of [...App.nodes]) {
      ...
      if (keeper.imageSrc && !keeper.imageEl) {
        App.restoreImageNode(keeper);
      }
