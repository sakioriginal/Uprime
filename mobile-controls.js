...
export class MobileControls{
  constructor(state,avatar,scene,root){this.state=state;this.avatar=avatar;this.scene=scene;this.root=root;this.spacecraftFlight=null;this.move={x:0,y:0};this.look={x:0,y:0};this.active={};this.ensureState();this.installSticks();this.installMouseLook();this.installMobileLayout()}
  setSpacecraftFlight(manager){this.spacecraftFlight=manager;return this}

  installMobileLayout(){
    const update=()=>{const coarse=matchMedia?.('(pointer: coarse)')?.matches??false,land=matchMedia?.('(orientation: landscape)')?.matches??false,active=!!(coarse&&land);document.body.classList.toggle('mobileLandscapeGame',active);if(active){const dial=document.querySelector('#universalDialOverlay');if(dial&&!dial.dataset.mobileCollapsed){dial.classList.add('collapsed');dial.dataset.mobileCollapsed='1'}}};
    update();window.addEventListener('resize',update,{passive:true});window.addEventListener('orientationchange',()=>setTimeout(update,120),{passive:true});
  }
