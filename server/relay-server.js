...
wss.on('connection', ws => {
  ...
  ws.on('message', buf => {
    ...
    if (msg.type === 'hello') {
      ...
      safeSend(ws, { type: 'welcome', room: roomId, clientId, isHost: room.hostId === clientId, peers: peers(room), seq: room.seq, hasSnapshot: !!room.snapshot, cloudSavedAt: room.updatedAt });
    ...
    if (msg.type === 'project-snapshot' && msg.snapshot) {
      room.seq++; room.snapshot = msg.snapshot; room.updatedAt = new Date().toISOString(); persist(ws._ue.roomId, room);
    ...
    if (msg.type === 'presence') { broadcast(room, { ...msg, clientId: ws._ue.clientId, name: ws._ue.name, playMode: ws._ue.playMode, team: ws._ue.team }, ws); return; }
  ...
  ws.on('close', () => {
    ...
    room.clients.delete(meta.clientId); broadcast(room, { type: 'peer-left', clientId: meta.clientId }); cleanupRoom(meta.roomId);
  });
});
