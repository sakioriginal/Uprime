...
import {UILayoutRTSManager} from "./workspace/ui-layout-rts.js";
...
installShopUi();
uiLayoutRts=new UILayoutRTSManager(state,{scene,avatar:avatarManager,planet:planetManager,resources:survivalResources,mobileUI:mobileGameUI,creator:creatorMode,onStatus:(t,type)=>status(t,type)});
...
function updateAvatarPositionHud(){
  ...
  state.controls=state.controls||{};if(state.controls.coordinateHud===undefined)state.controls.coordinateHud=false;
  hud.classList.toggle('coordHudHidden',state.controls.coordinateHud!==true);if(state.controls.coordinateHud!==true)return;
...
$("#coordinateHudToggle").onclick=()=>{state.controls=state.controls||{};state.controls.coordinateHud=state.controls.coordinateHud!==true;updateAvatarPositionHud();status(`座標HUD: ${state.controls.coordinateHud?"ON":"OFF"}`,"command")};
